...
from drafting_agent.claude_draft import try_claude_draft
...
def _display_remedy(remedy: str) -> str:
    labels = {
        ...
        "partial_coverage": "approval of the medically necessary covered services",
...
def _deterministic_draft(strategy: dict[str, Any]) -> dict[str, Any]:
    """Citation-grounded deterministic letter (no LLM)."""
...
def draft_letter(strategy: dict[str, Any]) -> dict[str, Any]:
    """Prefer Claude tool draft (grounded in strategy); fall back to deterministic."""
    drafted = try_claude_draft(strategy)
    if drafted is not None:
        return drafted
    return _deterministic_draft(strategy)
