...
from orchestrator.claude_verification import enrich_verification_report
from orchestrator.verification import verify_pipeline_artifacts
from personal_evidence_agent.claude_review import review_personal_evidence_bundle
from orchestrator.war_room_narrative import build_phase_narrative
...
def run_pipeline(
    ...
    war_room_bundle: dict[str, Any] = {
        "denial_intake": denial_intake,
        "personal_evidence": personal_evidence,
    }
    seed_narrative = build_phase_narrative(
        "seed_golden_artifacts",
        war_room_bundle,
        output_hint=f"artifacts dir seeded with intake + evidence tasks ({artifacts_dir})",
    )
    record("seed_golden_artifacts", "success", str(artifacts_dir), narrative=seed_narrative)

    record(
        "personal_evidence_agent",
        "running",
        notes=["Claude critical review: task vs. personal_evidence coherence"],
    )
    pe_review = review_personal_evidence_bundle(personal_evidence_task, personal_evidence)
    pe_path = artifacts_dir / "personal_evidence_claude_review.json"
    if pe_review:
        write_json(pe_path, pe_review)
    war_room_bundle["pe_review"] = pe_review or {}
    pe_narrative = build_phase_narrative(
        "personal_evidence_agent",
        war_room_bundle,
        output_hint=str(pe_path) if pe_review else "review skipped (no Claude / error)",
    )
    record(
        "personal_evidence_agent",
        "success",
        str(pe_path) if pe_review else None,
        notes=(["personal_evidence_claude_review.json written"] if pe_review else ["no Claude review artifact"]),
        narrative=pe_narrative,
    )

    # ── Contact agent ──────────────────────────────────────────────────────
    record("contact_agent", "running", notes=["Resolving missing fields → contact_actions"])
    ...
    war_room_bundle["contact_actions"] = contact_actions
    contact_narrative = build_phase_narrative("contact_agent", war_room_bundle, output_hint=str(contact_path))
    record("contact_agent", "success", str(contact_path), narrative=contact_narrative)

    # ── External evidence agent ────────────────────────────────────────────
    record("external_evidence_agent", "running", notes=["Retrieving external citations (Mongo / curated corpus)"])
    ...
    war_room_bundle["external_evidence"] = external_artifact
    external_narrative = build_phase_narrative(
        "external_evidence_agent",
        war_room_bundle,
        output_hint=str(external_path),
    )
    record(
        "external_evidence_agent",
        external_artifact.get("status", "success"),
        str(external_path),
        narrative=external_narrative,
    )

    # ── Appeal strategy agent ──────────────────────────────────────────────
    record("appeal_strategy_input", "running", notes=["Validating merged strategy contract"])
    ...
    war_room_bundle["strategy_warnings"] = warnings
    si_narrative = build_phase_narrative(
        "appeal_strategy_input",
        war_room_bundle,
        output_hint=str(strategy_input_path),
    )
    record("appeal_strategy_input", "success", str(strategy_input_path), notes=warnings, narrative=si_narrative)

    record("appeal_strategy_agent", "running", notes=["Synthesizing remedy + argument_chain via strategy engine"])
    ...
    war_room_bundle["strategy"] = strategy
    strat_narrative = build_phase_narrative(
        "appeal_strategy_agent",
        war_room_bundle,
        output_hint=str(strategy_path),
    )
    record("appeal_strategy_agent", "success", str(strategy_path), narrative=strat_narrative)

    # ── Drafting agent ─────────────────────────────────────────────────────
    record("drafting_agent_letter", "running", notes=["Drafting footnoted appeal letter"])
    try:
    ...
    war_room_bundle["draft_letter_excerpt"] = (drafted.get("appeal_letter") or "")[:4000]
    letter_narrative = build_phase_narrative(
        "drafting_agent_letter",
        war_room_bundle,
        output_hint=str(drafted_path),
    )
    record("drafting_agent_letter", "success", str(drafted_path), notes=draft_notes, narrative=letter_narrative)

    record("drafting_agent_packet", "running", notes=["Assembling appeal_packet + exhibits checklist"])
    packet = build_packet(strategy, drafted)
    packet_path = artifacts_dir / "appeal_packet.json"
    write_json(packet_path, packet)
    packet_narrative = build_phase_narrative(
        "drafting_agent_packet",
        war_room_bundle,
        output_hint=str(packet_path),
    )
    record("drafting_agent_packet", "success", str(packet_path), narrative=packet_narrative)

    # ── Verification ───────────────────────────────────────────────────────
    record("verification", "running", notes=["Cross-checking artifacts + enrichment"])
    ...
    verification_report = enrich_verification_report(verification_report)
    verification_path = artifacts_dir / "verification_report.json"
    write_json(verification_path, verification_report)
    war_room_bundle["verification_status"] = verification_report.get("status")
    verify_narrative = build_phase_narrative(
        "verification",
        war_room_bundle,
        output_hint=str(verification_path),
    )
    record("verification", verification_report["status"], str(verification_path), narrative=verify_narrative)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Meridian local pipeline from golden JSON artifacts."
    )
    parser.add_argument(
        "--case",
