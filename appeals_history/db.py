...
Uses the same cluster as the evidence-retrieval database (`MONGODB_DB`) but
addresses a separate database name so the two are isolated operationally.

    cluster/
        <MONGODB_DB>           <- evidence index (e.g. evidence_chunks)
        appeal_history        <- this module (appeal_records)

Required environment variables (add to your .env):
    MONGODB_URI                 connection string   (shared cluster)
...
# MongoClient manages its own connection pool; one instance can serve both the
# evidence DB and appeal_history.
# ─────────────────────────────────────────────────────────────────────────────
