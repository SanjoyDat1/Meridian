...
def generate_llm_queries(task: ExternalEvidenceTask, top_k: int = 8) -> tuple[list[EvidenceQuery], list[str]]:
    ...
    if os.getenv("ANTHROPIC_API_KEY"):
        ...
        try:
            from shared.claude_runtime import default_model

            raw = call_anthropic(prompt, default_model())
            response = parse_llm_response(raw)
            queries = normalize_llm_queries(task, response.queries, top_k=top_k)
            if queries:
                return queries, [
                    f"Anthropic Claude query intents merged with deterministic fallback (model={default_model()})."
                ]
            notes.append("Anthropic returned no usable queries; falling back to LLM_PROVIDER or deterministic.")
        except Exception as exc:
            notes.append(f"Anthropic query generation failed ({type(exc).__name__}); trying LLM_PROVIDER.")

    provider = os.getenv("LLM_PROVIDER", "").strip().lower()
    model = os.getenv("LLM_MODEL", "").strip()
    if not provider:
        return [], notes + ["LLM query generation skipped (no LLM_PROVIDER and no usable Anthropic result)."]
    if not model:
        return [], notes + ["LLM query generation skipped because LLM_MODEL is not configured."]
