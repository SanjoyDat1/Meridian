...
from claude_critique import citations_brief_for_critique, critique_external_bundle
from schemas import (
    CitationSource,
    EvidenceQuery,
    ExternalCitation,
...
def _mongo_client_kwargs(uri: str) -> dict[str, Any]:
    """Atlas/SRV URIs need a CA bundle; local mongod typically does not use TLS."""
    opts: dict[str, Any] = {"serverSelectionTimeoutMS": 12000}
    lowered = uri.lower()
    if (
...
class MongoEvidenceStore:
    def __init__(self) -> None:
        ...
        client = MongoClient(uri, **_mongo_client_kwargs(uri))
...
def retrieve_external_evidence(task: ExternalEvidenceTask, top_k: int = 8) -> ExternalEvidenceArtifact:
    ...
    critique = critique_external_bundle(task.model_dump(mode="json"), citations_brief_for_critique(citations))
    extra_notes: list[str] = []
    if critique and isinstance(critique, dict):
        ra = critique.get("retrieval_assessment")
        if ra and not critique.get("error"):
            extra_notes.append(f"Claude evidence critique: {ra}")
    ...
    return ExternalEvidenceArtifact(
        ...
        provenance={
            ...
            "claude_critique_present": bool(critique and not critique.get("error")),
        },
        data=ExternalEvidenceData(
            ...
            claude_critical_review=critique,
        ),
    )
